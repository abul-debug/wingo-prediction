...
// Single Environment Variable for Firebase Key
...
module.exports = async (req, res) => {
  ...
  try {
    if (req.query.action === 'reset') {
      ...
      return res.status(200).json({ status: "success", message: "DB Reset Done" });
    }
    ...
    if (!data) {
      return res.status(200).json({ status: "pending", message: "Fetching data... Refresh page in 3 seconds." });
    }
    ...
    if (history.length < 2) {
      return res.status(200).json({ status: "pending", message: "Need at least 2 periods to calculate." });
    }
    ...
    if (nextNumbers.length === 0) {
      return res.status(200).json({
        ...
        message: "First time occurrence today."
      });
    }
    ...
    return res.status(500).json({ status: "error", error_message: error.message });
  }
};
